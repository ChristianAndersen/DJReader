...
- (void)_renderPages:(CGContextRef)context rect:(CGRect)rect
{
	...
	for (NSInteger i = 0; i < _numberOfPages; i++) {
		...
		if (i == _displayedPage) {
			...
			fillColor = _currentPageIndicatorTintColor ? _currentPageIndicatorTintColor : ControllerDefalutColor;
			image = _currentPageImages[indexNumber];
			if (nil == image) {
				image = _currentPageIndicatorImage;
			}
		} else {
			fillColor = _pageIndicatorTintColor ? _pageIndicatorTintColor : [ControllerDefalutColor colorWithAlphaComponent:0.3f];
